...
def make_tray():
    tray = TrayIcon(on_toggle=lambda: None, on_exit=lambda: None)
    tray._icon = FakeIcon()
    return tray
...
def test_就绪后会补上之前跳过的更新():
    ...
    assert tray._light is Light.RED


def test_灯色不变时不重建图标():
    tray = make_tray()
    tray._icon.visible = True
    tray.update(Light.RED, "忙碌 · 1 会话")
    first = tray._icon.icon
    tray.update(Light.RED, "忙碌 · 2 会话")
...
def test_灯色变化时重建图标():
    ...
    tray.update(Light.RED, "忙碌")
    first = tray._icon.icon
    tray.update(Light.GREEN, "待机")
    assert tray._icon.icon is not first
    assert tray._light is Light.GREEN
