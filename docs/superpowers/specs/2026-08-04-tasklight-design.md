...
以下两点是明知的不完美，已确认按现状实现，不必在编码时重新权衡：

1. **圆角靠 `-transparentcolor` 抠色**，属 Windows 专有技巧，某些显卡或缩放设置下可能露出品红边。若实测出现，退路是改成直角窗口，不折腾其他方案。
2. **`bg_since` 只记最近一次后台命令的时刻**，不存时间戳列表。已知失效场景：同一会话先起长任务、再起短任务，短任务结束后长任务仍在跑 —— 此时长任务的进程创建时间早于新 `bg_since`，会被误判为已完成而转绿。判断为小概率场景，按 YAGNI 不处理。
